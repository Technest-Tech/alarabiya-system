<aside 
    ...
    <div class="flex h-16 items-center justify-between px-6 border-b border-indigo-600 flex-shrink-0">
        <a href="{{ Auth::user()->canAccessAdminDashboard() ? route('admin.dashboard') : route('teacher.dashboard') }}" class="flex items-center space-x-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-white/10 backdrop-blur-sm overflow-hidden">
                <img src="{{ asset('logo.png') }}" alt="Alarabiya Academy Logo" class="h-full w-full object-contain">
            </div>
            <div>
                <h1 class="text-lg font-bold">Alarabiya Academy</h1>
            </div>
        </a>
        <button 
            x-data
    ...
    <nav class="mt-6 px-3 space-y-1 flex-1 overflow-y-auto pb-4">
        @if(Auth::user()->isAdmin())
            ...
            <a href="{{ route('admin.teacher-adjustments.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-150 {{ request()->routeIs('admin.teacher-adjustments.*') ? 'bg-white text-indigo-700 shadow-lg' : 'text-white hover:bg-white/10' }}">
                <svg class="mr-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
