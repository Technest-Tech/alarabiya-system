<x-app-layout pageTitle="Packages Overview">
            ...
            @foreach($students as $student)
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-all duration-200">
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button type="button"
                                x-data=""
                                x-on:click="$dispatch('open-modal', 'report-modal-{{ $student->id }}')"
    ...
    <!-- Report Modals -->
    @foreach($students as $student)
        <x-modal name="report-modal-{{ $student->id }}" maxWidth="md">
            <form method="GET" action="{{ route('admin.packages.report', $student) }}" class="p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="from_date_{{ $student->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            From Date
                        </label>
                        <input
                            type="date"
                            id="from_date_{{ $student->id }}"
                            name="from_date"
                    ...
                    <div>
                        <label for="to_date_{{ $student->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            To Date
                        </label>
                        <input
                            type="date"
                            id="to_date_{{ $student->id }}"
                            name="to_date"
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button
                        type="button"
                        x-on:click="$dispatch('close-modal', 'report-modal-{{ $student->id }}')"
                        class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                        Cancel
                    </button>
                    <button
                        type="submit"
                        class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-indigo-600 rounded-lg shadow-sm hover:from-purple-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                        Download Report
                    </button>
                </div>
